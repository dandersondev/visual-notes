...
- **Leaving a room no longer throws away your access to it.** Leave now simply stops syncing, and the board offers **Rejoin**. Discarding a room is a separate **Forget** action that asks first. This mattered most to room owners: leaving used to delete the only copy of your owner access, and a room owner cannot be let back in by an invitation, so leaving your own room locked you out of it permanently.
- **Owners can recover a room stored on their own device.** While hosting, a board with no room offers **Reopen my room**, listing the rooms held on this machine and restoring your owner access to the one you pick. Everyone else keeps the access they already have. This works only on the hosting machine, deliberately — an invitation contains the server credential, so holding one cannot be what proves ownership.
  - Rooms are listed by the name of the board they were created from, with the cards and people in them, when each was last active, and the internal identifier shown quietly underneath rather than as the heading. Rooms made before this release did not record that name, so Visual Notes finds it — by the board still holding the room, or by recognising the board the room's contents came from. Only a room whose board no longer exists falls back to describing what is on it.
